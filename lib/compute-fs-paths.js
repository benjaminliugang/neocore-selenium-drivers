...
let sys = require('./getSystem')();
const basePath = path.join(__dirname, '../', 'drivers', process.env.SYSTEM);
...
function computeFsPaths(opts) {
  ...
  if (opts.drivers.edge) {
    fsPaths.edge = {
      installPath: path.join(
        opts.basePath,
        'edgedriver',
        opts.drivers.edge.version + '-MicrosoftEdgeDriver.exe',
      ),
    };
  }
  ...
  fsPaths = Object.keys(fsPaths).reduce(function computeDownloadPath(newFsPaths, name) {
    ...
    } else if (name === 'firefox' && sys !== 'win32') {
